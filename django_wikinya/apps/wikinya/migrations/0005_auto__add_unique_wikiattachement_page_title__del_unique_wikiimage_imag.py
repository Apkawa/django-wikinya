...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'WikiImage', fields ['image', 'page']
        db.delete_unique('wikinya_wikiimage', ['image', 'page_id'])

        # Adding unique constraint on 'WikiAttachement', fields ['page', 'title']
        db.create_unique('wikinya_wikiattachement', ['page_id', 'title'])

        # Adding unique constraint on 'WikiImage', fields ['page', 'title']
        db.create_unique('wikinya_wikiimage', ['page_id', 'title'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'WikiImage', fields ['page', 'title']
        db.delete_unique('wikinya_wikiimage', ['page_id', 'title'])

        # Removing unique constraint on 'WikiAttachement', fields ['page', 'title']
        db.delete_unique('wikinya_wikiattachement', ['page_id', 'title'])

        # Adding unique constraint on 'WikiImage', fields ['image', 'page']
        db.create_unique('wikinya_wikiimage', ['image', 'page_id'])
    ...
    models = {
        ...
        'wikinya.wikiattachement': {
            'Meta': {'unique_together': "[('title', 'page')]", 'object_name': 'WikiAttachement'},
        ...
        'wikinya.wikiimage': {
            'Meta': {'unique_together': "[('title', 'page')]", 'object_name': 'WikiImage'},
            'author': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']"}),
            'datetime_create': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'datetime_update': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
